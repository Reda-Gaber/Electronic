...
export default function AdminOrderDetailPage() {
  ...
  // حالة محلية للحالة والملاحظات — تُعبّأ بعد جلب الطلب فعلياً
  const [currentStatus, setCurrentStatus] = useState<OrderStatus>('pending')
  const [adminNotes, setAdminNotes] = useState('')
  // رسوم الشحن كنص عشان تسمح بحقل فاضي (يعني "لسه ما اتحددتش")
  ...
  useEffect(() => {
    ...
    Promise.all([adminFetchOrderById(id), adminFetchOrders()])
      .then(([fetchedOrder, allOrders]) => {
        if (!isMounted) return
        setOrder(fetchedOrder)
        setCurrentStatus(fetchedOrder.status)
        setAdminNotes(fetchedOrder.adminNotes ?? '')
        setShippingFeeInput(
          fetchedOrder.shippingFee != null ? String(fetchedOrder.shippingFee) : '',
        )
        setShortCode(getShortOrderCode(fetchedOrder.id, allOrders))
      })
      .catch(() => {
  ...
  // حفظ الحالة الجديدة والملاحظات فعلياً عبر الـ API
  const handleSave = async () => {
    ...
    try {
      const updated = await adminUpdateOrder(order.id, {
        ...
        adminNotes,
        shippingFee: shippingFeeInput.trim() === '' ? null : Number(shippingFeeInput),
      })
      setOrder(updated)
      setShippingFeeInput(updated.shippingFee != null ? String(updated.shippingFee) : '')
      setSaveMessage('تم حفظ التغييرات بنجاح')
    } catch (err) {
      setSaveMessage(
        err instanceof ApiClientError ? err.message : 'تعذر حفظ التغييرات، حاول تاني',
      )
    } finally {
      setIsSaving(false)
  ...
  return (
    <div>
      ...
      <div className="mb-6 flex flex-wrap items-center gap-3">
        ...
        <div>
          <h1 className="font-display text-xl font-extrabold text-on-surface">{shortCode}</h1>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
          ...
          <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
            ...
            <div className="mt-4 space-y-2 border-t border-outline-variant/60 pt-4 text-sm">
              ...
              <div className="flex justify-between text-on-surface-variant">
                ...
                <span>
                  {order.shippingFee != null ? formatPrice(order.shippingFee) : 'لسه ما اتحددتش'}
                </span>
              </div>
        ...
        <div className="space-y-6">
          ...
          {/* رسوم الشحن — يحددها الأدمن يدوياً بعد تأكيد شركة الشحن للتكلفة */}
          <div className="rounded-2xl border border-outline-variant bg-surface-container-lowest p-5">
            <h2 className="mb-3 flex items-center gap-2 font-display text-base font-bold text-on-surface">
            ...
            <div className="flex items-center gap-2">
              ...
              />
              <span className="shrink-0 text-xs text-on-surface-variant">ج.م</span>
            </div>
            <p className="mt-1.5 text-xs text-on-surface-variant">
              سيبها فاضية لو لسه شركة الشحن ما أكدتش التكلفة — العميل عرفناه إن رسوم الشحن
              هتتأكد معاه تليفونياً
            </p>
          </div>
