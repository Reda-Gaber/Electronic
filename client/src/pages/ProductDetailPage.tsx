...
export default function ProductDetailPage() {
  ...
  // جلب المنتج والمنتجات ذات الصلة عند تغيير الـ slug
  useEffect(() => {
    if (!slug) return
    let isMounted = true
    setIsLoading(true)
    ...
    fetchProductBySlug(slug)
      .then(async (fetchedProduct) => {
        const related = await fetchProducts({ category: fetchedProduct.categoryId })
        if (!isMounted) return
  ...
  // منتجات من نفس التصنيف — تُعرض أسفل الصفحة (جاهزة بالفعل في relatedProducts state)
