...
export default function OrderTrackingPage() {
  ...
  const handleSearch = async (e: FormEvent) => {
    ...
    setIsSearching(true)
    try {
      const found = await trackOrder(referenceNumber.trim(), phone.trim())
      setResult(found)
    } catch {
      setResult(undefined)
    } finally {
    ...
    <div className="container-main flex justify-center pb-24 pt-4 md:pb-12 md:pt-6">
      <div className="w-full max-w-lg">
        ...
        >
          ...
          >
            <span
              className={`material-symbols-outlined text-lg ${isSearching ? 'animate-spin' : ''}`}
            >
              {isSearching ? 'progress_activity' : 'search'}
            </span>
            {isSearching ? 'جاري البحث...' : 'ابحث عن طلبي'}
          </button>
        </form>
