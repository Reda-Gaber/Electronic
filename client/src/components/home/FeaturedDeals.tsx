 ...
 *
 * الوظيفة: عرض منتجات مخفضة في شبكة
 * Props: لا يوجد — يفلتر المنتجات المنشورة ذات originalPrice من الـ API
...
export default function FeaturedDeals() {
  ...
  useEffect(() => {
    ...
    fetchProducts()
      .then((products) => {
        if (isMounted) {
          setDealProducts(products.filter((p) => p.originalPrice).slice(0, 6))
        }
      })
      .catch(() => {
